                ...
                <div class="card w-100">
                    <div class="card-body p-4">
                        <livewire:module-search-component wire:id="{{rand()}}" wire:key="{{rand()}}"
                            :className="$modelClass=App\Models\DepoGuest::class"
                            :classAssocName="$modelAssocClass=App\Models\DepoGroup::class"
                            :searchParams="['depo_guest_name','depo_guest_contact','depo_identity','depo_guest_email']"
                            :renderComponent="['depo_guest_name','depo_rep_name','depo_guest_designation','depo_identity','depo_guest_contact','depo_guest_email','badge_type']"
                            searchAssocParams="depo_uid" />
                    </div>
                </div>
            </div>
        </div>
...
<div class="row">
    <div class="d-flex col-md-4 col-sm-2">
        <div class="card overflow-hidden">
            <div class="card-body p-4">
                <h5 class="card-title text-center mb-9 fw-semibold">Total Host Guest Updated</h5>
        ...
        <div class="card-body p-4">
            ...
            <div class="table-responsive">
                <table id="table" data-filter-control-multiple-search="true"
                    ...
                    data-url="{{route('request.getDepoGroups')}}?status={{request()->get('status') ?? 1}}">
                    <thead>
                        <tr>
                            <th data-filter-control="input" data-field="SNO" data-formatter="operateSerial">S.No.</th>
                            @if(session()->get('user')->roles[0]->name === "admin" ||
                            session()->get('user')->roles[0]->name === "depo"|| session()->get('user')->roles[0]->name
                            ...
                            <th data-filter-control="input" data-field="category.name" data-sortable="true"
                            ...
                            <th data-filter-control="input" data-field="depo_status" data-sortable="true"
                                data-formatter="operateStatus">Status</th>
...
<script>
    ...
    function operateStatus(value, row, index) {
        if (value) {
            return `<button class="btn btn-badar" style="text-transform: capitalize;" onclick='statusToggle(this,"${row.uid}",${value})'><span style="display:none;">${value?'active':'block'}</span><i class="ti ti-x" style="font-size:22px; widht:24px; height:24px;"></i></button>`;
        } else {
            return `<button class="btn btn-success" style="text-transform: capitalize;" onclick='statusToggle(this,"${row.uid}",${value})'><span style="display:none;">${value?'active':'block'}</span><i class="ti ti-check" style="font-size:22px; widht:24px; height:24px;"></i></button>`;
        }
    }
    ...
    function operateSerial(value, row, index) {
        ...
        function statusToggle(e,uid,value){
            e.disabled = true;
            const $tableForRefresh = $('#table');
            let route = '{{route("request.depoGuest.statusUpdate", ":uid")}}'.replace(":uid",uid);
            axios({
                method: 'POST',
                url: route,
                data: {'depo_status': value == 1 ? 0 : 1},
                headers: {
            ...
            .then(response => {
                ...
                <div class="alert alert-${response.data ? 'success' : 'danger'} alert-dismissible fade show" role="alert">
                    <strong>Status Has Been Updated</strong>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>`;
            ...
            .finally(() => {
                e.disabled = false;
                $tableForRefresh.bootstrapTable('refresh');
            });
    }
